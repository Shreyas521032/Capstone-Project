...
def load_data():
    try:
        df1 = pd.read_excel("Project/cancer_website_survey_dataset.xlsx")  
        df2 = pd.read_excel("Project/website_survey_feedback_responses.xlsx")
...
# Multiple Regression
    ...
    ordinal_mappings = {
    'Q14 Most Valuable Feature': {
        'Information': 1,
        'Support': 2, 
        'Tools': 3,
        'Resources': 4
    },
    # Add other ordinal columns here
}
    predictors_data = data[predictors].copy()
    for col, mapping in ordinal_mappings.items():
        if col in predictors_data.columns:
            predictors_data[col] = predictors_data[col].map(mapping)
    # Correlation heatmap for predictors
    st.markdown("<h4>Correlation Between Predictors</h4>", unsafe_allow_html=True)
    numeric_predictors = [col for col in predictors if data[col].dtype in ['int64', 'float64']]
    if numeric_predictors:
        corr = data[numeric_predictors].corr()
        fig = px.imshow(corr, 
                   text_auto='.2f',
                   color_continuous_scale='Blues',
                   title="Correlation Between Numeric Predictors")
        fig.update_layout(height=500)
        st.plotly_chart(fig, use_container_width=True)
    corr = data[predictors].corr()
    fig=    fig = px.imshow(corr, 
                   text_auto='.2f',
                   color_continuous_scale='Blues')
    fig.update_layout(height=500)
    st.plotly_chart(fig, use_container_width=True)
